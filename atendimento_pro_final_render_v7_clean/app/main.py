...
@login_required
def support_center():
    selected_client_id = request.args.get("client", type=int)
    page = request.args.get("page", 1, type=int)
    search = request.args.get("q", "").strip()

    if request.method == "POST":
        client = Client.query.get(selected_client_id) if selected_client_id else None
        create_support_entry(client)
        return redirect(url_for("main.support_center"))

    clients = Client.query.order_by(Client.name.asc()).all()
    query = Attendance.query.outerjoin(Client)

    if selected_client_id:
        query = query.filter(Attendance.client_id == selected_client_id)
...
def create_support_entry(client: Client | None = None) -> None:
    ...
    attendance_dt = parse_datetime_local(attended_at) or local_now()
    ...
    attendance = Attendance(
        ...
        title=title,
        description=description,
        attended_at=attendance_dt,
        next_follow_up=parse_date(follow_up),
    )
