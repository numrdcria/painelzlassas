...
def migrate_attendances_if_needed() -> None:
    ...
    if not {"contact_name", "contact_phone"}.issubset(columns) and db.engine.url.get_backend_name().startswith("sqlite"):
        with db.engine.begin() as conn:
            conn.execute(text("""
                CREATE TABLE attendances_new (
                    ...
                    service_status VARCHAR(20) NOT NULL DEFAULT 'aberto',
            ...
            conn.execute(text("""
                INSERT INTO attendances_new (
                    id, client_id, user_id, contact_name, contact_phone, platform, issue_type, device_type, service_status, priority, title, description,
                    attended_at, next_follow_up, created_at, updated_at
                )
                SELECT a.id, a.client_id, a.user_id,
                       COALESCE(c.name, ''), COALESCE(c.whatsapp, ''), NULL, NULL, NULL, 'aberto', 'normal',
        ...
        columns = {col["name"] for col in inspect(db.engine).get_columns("attendances")}
